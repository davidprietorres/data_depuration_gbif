...
##Si se usa Linux, es necesario instalar lo siguiente primero con el gestor
##de paquetes del sistema (por ejemplo, apt en Debian/similares):
#libxml2-dev libcurl4-openssl-dev gdal-config libgdal-dev libfontconfig1-dev
#libharfbuzz-dev libfribidi-dev libudunits2-dev libabsl-dev cmake
...
species1<- gbif("Fagus", "grandiflora*", geo=FALSE)##permite vincularse a la pagina GBIF y descargar datos, para ello es necesario escribir el nombre de la especie separando el g?nero del epiteto (el asterisco es para no descartar datos por las "subspecies").
View(species1)##permite visualizar la tabla de datos descargada, conformada por 4233 observaciones con 159 variables de informacion para cada registro segun el estandar de datos de Darwin Core.
