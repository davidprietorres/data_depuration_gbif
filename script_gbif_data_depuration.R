...
#############################################################################################
install.packages(c("rgbif","TeachingDemos","dismo","biomod2","sp","raster","rgeos","maptools", "rgdal", "usdm", "ENMeval", "spocc", "corrplot", "XML", "ecospat", "dplyr", "reshape", "CoordinateCleaner", "modeest"))
...
View(species1)##permite visualizar la tabla de datos descargada, conformada por 4,722 observaciones con 153 variables de información para cada registro según el estándar de datos de Darwin Core.
...
#3.Crear un archivo en formato ".CSV" con la información básica necesaria para cada registro, incluyendo: "código GBIF", "institución de procedencia", "número de catálogo/colección", "taxonomía" y "coordenadas geográficas", así como "año", "país" y "región" de colecta. NOTA: para este ejercicio solo se estan seleccionando 15 columnas de las 153 disponibles en GBIF, no obstante esto puede modificarse de acuerdo a la necesidades y preferencias de cada investigador
...
View(data_cleaned1)##permite visualizar la tabla de datos descargada, conformada por 4,722 observaciones con 153 variables de información para cada registro según el estándar de datos de Darwin Core.

##Guardar el archivo creado para los datos datos descargados y filtrados
setwd("E:/escritorio/Trabajo/Articulos pendientes/Manual de Biogeografía/adquisición_datos/")##permite indicar el directorio dentro del computador donde queremos guardar el archivo ".csv" contentivo de la información
...
data4 <- subset(data_cleaned1, year > 1969 & year < 2001)###selecciona los datos de 1970 al 2000 (coincidentes con las capas ambientales que se usaran en los análisis subsecuente de modelado)
data5 <- subset(data_cleaned1, year > 2000)###selecciona los datos del 2001 al presente (NO coincidentes con las capas ambientales que se usaran en los análisis subsecuente de modelado)
...
datos6<-data4[(data4$lat>17.6),]#elimina los datos con latitud menor a 17.6°
plot(wrld_simpl, xlim = c(-95, -85), ylim = c(18,20), axes = TRUE, col = "light blue")###me permite ver un mapa con una escala mas cercana a la distribucion de mis datos
...
##Guardar el archivo creado para los datos datos descargados y filtrados
setwd("E:/escritorio/Trabajo/Articulos pendientes/Manual de Biogeografía/adquisición_datos/")##permite indicar el directorio dentro del computador donde queremos guardar el archivo ".csv" contentivo de la información
write.csv(data7, file = "Amazona_xantholora_1970-2000.csv") ###es el comando utilizado para guardar el archivo de nombre "Amazona_xantholora" con extensión ".csv"
write.csv(data8, file = "Amazona_xantholora_2001-2020.csv") ###es el comando utilizado para guardar el archivo de nombre "Amazona_xantholora" con extensión ".csv"
...
buffer = quantile(DistanciaPuntos, probs = 0.05, na.rm = TRUE)#me da el valor máximo observado entre los datos para calcular la distancia que se usara como área BUFFER para seleccionar cuales datos entre 2001-2020 entraran en nuestra matriz (acá el valor es 0.222 Grados decimales = 26.65)
...
plot(wrld_simpl, xlim = c(-95, -85), ylim = c(18,20), axes = TRUE, col = "light blue")###dibujar un mapa estableciendo 
...
setwd("E:/escritorio/Trabajo/Articulos pendientes/Manual de Biogeografía/adquisición_datos/coberturas_presente/") ###directorio donde estan las capas climaticas del presente 
pca_path <- list.files(".",pattern = "*.asc$",full.names = T)###crea el stack de las 19 variables ambientales
...
species<-data7$species ## seleccionar la columna del nombre de la especie para los datos 1970-2000
lat<-as.numeric(data7$lat)## seleccionar la columna del nombre de la latitud para los datos 1970-2000
lon<-as.numeric(data7$lon)## seleccionar la columna del nombre de la longitud para los datos 1970-2000
...
presencias_clima <- data.frame(extract(capas_presente, points_occ2000 [,11:12]))###extrae los valores climaticos para cada uno de los puntos de presencia
...
#4.realizar el filtrado ambiental de los datos para las localidades obtenidas entre 2001 y 2020
##extraemos los valores ambientales para esas localidad y omitimos los datos sin informacion
presencias2020_clima <- data.frame(extract(capas_presente,points_occ2020[,11:12]))###extrae los valores climaticos para cada uno de los puntos de presencia "seleccionados" de 2001-2020
presencias2020_clima2<-data.frame(points_occ2020,presencias2020_clima)##crear una tabla de datos con los valores climaticos para mis registros "seleccionados" de 2001-2020
...
data_dem <- subset(presencias2020_clima3, dem > dem_min & dem < dem_max)#elimina datos con rango altitudinal superior a los 203msnm
data2020_1 <- subset(data_dem, bio_1 > bio1_min & bio_1 < bio1_max)#elimina datos con temperatura promedio anual inferior a 24.89°C y superiores a 26.86°C
data2020_12 <- subset(data2020_1, bio_12 > bio12_min & bio_12 < bio12_max)#elimina datos con precipitación anual inferior a 860.55mm y superiores a 1701.04mm
data2020_15 <- subset(data2020_12, bio_15 > bio15_min & bio_15 < bio15_max)#elimina datos con precipitación estacional inferior a 42.48 y superiores a 73.76

data_1970_2000_fin <- presencias_clima3###corresponde al archivo final de datos para los años entre 1970 y 2000 
data_2001_2020_fin <- data2020_15###corresponde al archivo final de datos para los años entre 2001 y 2020

#5. Armar el único archivo .CSV con los registros validados y depurados de la especie 
species<- c(data_1970_2000_fin$species, data_2001_2020_fin$species)##selecciona y une las columnas "species" de ambos archivos
lat<-as.numeric(c(data_1970_2000_fin$lat, data_2001_2020_fin$lat))##selecciona y une las columnas "latitud" de ambos archivos
lon<-as.numeric(c(data_1970_2000_fin$lon, data_2001_2020_fin$lon))##selecciona y une las columnas "longitud" de ambos archivos
...
setwd("E:/escritorio/Trabajo/Articulos pendientes/Manual de Biogeografía/adquisición_datos/")##carpeta donde guardar el archivo final de los datos
