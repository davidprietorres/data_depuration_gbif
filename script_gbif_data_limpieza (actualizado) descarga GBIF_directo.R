...
setwd("D:/proyectos_sigs/biogeografía2025-2/data_depuration_gbif-main/0047941-250811113504898/")###directorio de la carpeta donde esta el archivo csv con los datos descargados.
gbif_download = readr::read_tsv("0047941-250811113504898.csv")##colocar el nombre del archivo csv de la especie que se descargo desde la pagina de GBIF

gbif_download2 = gbif_download %>%
  setNames(tolower(names(.))) %>% # set lowercase column names to work with CoordinateCleaner
  filter(occurrencestatus  == "PRESENT") %>%#selecciona datos de solo presencia en el presente (nada de fosiles)
  filter(!basisofrecord %in% c("FOSSIL_SPECIMEN","LIVING_SPECIMEN")) %>%#selecciona datos de solo presencia en el presente (nada de fosiles)
  filter(!establishmentmeans %in% c("MANAGED", "INTRODUCED", "INVASIVE", "NATURALISED")) %>%#elimina todo lo que sea introducido o como datos de otros lados
  filter(year >= 1950) %>% #selecciona datos de solo presencia de 1950 en adelante
  filter(coordinateprecision < 0.01 | is.na(coordinateprecision)) %>% #selecciona datos de alta precisión poco incertidumbre
  glimpse() # look at results of pipeline

species1<- gbif_download2##me genera el archico con el cual voy a trabajar de datos... 
names(species1)##permite visualizar la tabla de datos descargada, conformada por 4233 observaciones con 159 variables de informacion para cada registro segun el estandar de datos de Darwin Core.

#2.Limpieza automatica b?sica de los datos: quitar observaciones con informacion incompleta
data1 <- subset(species1, !is.na(decimallongitude) & !is.na(decimallatitude))#quita todos los datos que no tienen coordenadas geograficas
data2 <- subset(data1, !is.na(year))#quita todos los datos que no tienen informacion del ano (fecha)
data4 <- subset(data2, (!(decimallatitude == 0 | decimallongitude == 0)))
data3 <- subset(data4, !is.na(stateprovince))#quita todos los datos que no tienen informacion del estado/localidad de colecta
...
institutionCode<-data3$collectioncode###seleciona la informaci?n sobre el "instituci?n de procedencia" para cada dato.
...
lat<-data3$decimallatitude###seleciona la informaci?n sobre la "Latitud" que corresponde a la localidad de muestreo de cada dato.
lon<-data3$decimallongitude###seleciona la informaci?n sobre la "Longitud" que corresponde a la localidad de muestreo de cada dato.
year<-data3$year###seleciona la informaci?n sobre el "A?O" de colecta o muestreo de cada dato.
country<-data3$countrycode###seleciona la informaci?n sobre el "pa?s" de la localidad de muestreo de cada dato.
region<-data3$stateprovince###seleciona la informaci?n sobre el "estado o provincia" de la localidad de muestreo de cada dato.
...
write.csv(data_cleaned1, file = "Troglodytes_aedon_gbif_original.csv", row.names = F) ###es el comando utilizado para guardar el archivo de nombre "Troglodytes_aedon" con extensi?n ".csv"

###Tabla concatenada de GBIF mas otros museos####
data_cleaned2 <- read.csv2("D:/proyectos_sigs/biogeografía2025-2/data_depuration_gbif-main/datos_descargados/Troglodytes_aedon_gbif_original.csv", sep = ",", header = TRUE) 
...
plot(world, xlim = c(-128, -34), ylim = c(-55,60), col = "lightblue", border = "gray40")###dibujar un mapa estableciendo un acercamiento al area geografica de distribucion de los datos obtenidos para la especie. Los valores en xlim corresponden a las longitudes observadas en los regitros, mientras que ylim corresponde a las latitudes
...
data7 <- clean_dup(data4,longitude = "lon",latitude = "lat",threshold = 0.08333)###datos 1970-2000 espacialmente limpios. En este caso de los  87 puntos que se tenian originalmente ahora solo quedan 27. la limpieza se hizo a 10km de resolucion
data8 <- clean_dup(data5,longitude = "lon",latitude = "lat",threshold = 0.08333)###datos 2001-2020 espacilamente limpios. En este caso de los  3701 puntos que se tenian originalmente ahora solo quedan 267. la limpieza se hizo a 10km de resolucion
...
buffer=(mean(DistanciaPuntos)*2)/100 #me da el valor m?ximo observado entre los datos para calcular la distancia que se usara como ?rea BUFFER para seleccionar cuales datos entre 2001-2020 entraran en nuestra matriz. En este caso el valor obtenido es 0.5549 grados, equivalentes a 66.58km
...
plot(world, xlim = c(-128, -34), ylim = c(-55,60), col = "lightblue", border = "gray40")###dibujar un mapa estableciendo 
...
data_cleaned2$species_name <- "Troglodytes_aedon"#crea una columna/variable colocando el nombre de la especie
...
windows()
plot(world, xlim = c(-128, -34), ylim = c(-55,60), col = "lightblue", border = "gray40")###me permite ver un mapa con una escala mas cercana a la distribucion de mis datos
...
write.csv(data10, file = "Troglodytes_aedon_limpios.csv", row.names = F)###es el comando utilizado para guardar el archivo de nombre "Troglodytes_aedon" con extension ".csv" En total quedaron 248 datos
...
st_write(shape_sf, "Troglodytes_aedon.shp", driver = "ESRI Shapefile")#guarda el archivo en formato shapefile de puntos con el nombre entre comillas
